...
int main (int argc, char *argv[])
{
  ...
  std::ofstream myfile;
  myfile.open ("BE1.txt");
  int sizes[5] = { 1, 5, 10, 15, 20 }; 
  int throughput_results[5] = {0, 0, 0, 0, 0};
...
for (int i = 0; i < 5 ; i++ ) { 
      ...
      for (uint32_t index = 0; index < nWifi; ++index)
        {
          ...
          onOffHelper.SetAttribute ("DataRate", DataRateValue (M*1000000));
  ...
  std::cout << nWifi << "\t" << "Throughput: " << throughput << " Mbit/s" << std::endl;

...
int main (int argc, char *argv[])
{
  ...
  std::ofstream myfile;
  myfile.open ("BE1.txt");
  double rates[5] = { 0.5, 1, 1.5, 2, 2.5}; 
  double throughput_results[5] = {0, 0, 0, 0, 0};
...
for (int i = 0; i < 5 ; i++ ) { 
  ...
  std::uniform_real_distribution<double> y_distribution(0.0, 1.0);
        

  for (uint32_t i = 0; i < nWifi; i++)
    {
      double x_pos = x_distribution(generator);
      double y_pos = y_distribution(generator);
      ...
      for (uint32_t index = 0; index < nWifi; ++index)
        {
          ...
          onOffHelper.SetAttribute ("DataRate", DataRateValue (rates[i]*1000000));
  ...
  std::cout << rates[i] << "\t" << "Throughput: " << throughput << " Mbit/s" << std::endl;
  throughput_results[i] = throughput;
  
  myfile << nWifi << "\t" << throughput_results[i] <<"\n";
}
